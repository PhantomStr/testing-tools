package com.phantomstr.testing.tool.utils.wait;
...
import static com.phantomstr.testing.tool.utils.time.TimeUtils.humanReadableFormat;
...
public class WaitTaskUtils {
    ...
    public static <O> O waitTask(Callable<O> callable,
        ...
        if (result == null) {
            ...
            executor.shutdown();
            throw new TimeoutException(format("%s (Timeout %s)", onTimeout, humanReadableFormat(Duration.of(msecTimeout, MILLIS))));
        }
        return result;
    }
